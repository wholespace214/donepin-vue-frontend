  ...
  <v-app v-resize="onResize" id="inspire">
    <v-app-bar app flat class="pa-0 mb-5" height="">
      <div class="flex w-full flex-col">
        <DashboardHeader :title="'team'" @openSideBar="sidebar = !sidebar" :SideBar="sidebar" />
        <DashboardMenu :menu="teamMenu" />
      </div>
    </v-app-bar>
...
import useSidebarSettings from "~/controller/Dashboard/SidebarSettings";
...
export default defineComponent({
  setup() {
    ...
    const { teamMenu, hamMenu } = allMenu();

    const { sidebarMenu, favoriteProject } = SidebarPage();

    const { onResize, ipadUpSize } = useResize();

    const { filterSidebar } = useSidebarSettings();

    filterSidebar(sidebarMenu);
