  ...
  <v-app v-resize="onResize" id="inspire">
    <v-app-bar app flat class="pa-0 mb-5" height="">
      <div class="flex w-full flex-col">
        <DashboardHeader :title="title" @openSideBar="sidebar = !sidebar" :SideBar="sidebar" />
...
export default defineComponent({
  setup() {
    ...
    const { financeMenu, invoiceMenu } = allMenu();

    const { onResize, ipadUpSize } = useResize();

    const route = useRoute();

    const title = computed(() => (route.value.path == "/finance" ? "finance" : "invoice"));

    const menu = computed(() => {
      let getSplitRoute: any | number = route.value.path.split("/");

      switch (getSplitRoute[getSplitRoute.length - 1]) {
        case "addinvoice":
          return invoiceMenu.value;
        case "recuiringInvoice":
          return invoiceMenu.value;
        case "timeloginvoice":
          return invoiceMenu.value;
        default:
          return financeMenu.value;
      }
    });
