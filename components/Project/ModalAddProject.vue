<template>
  ...
  <div
    :class="addProject ? ' -translate-x-0' : 'translate-x-full'"
    class="
      bg-white
      w-full
      sm:w-96
  ...
  >
    <div id="top" class="flex flex-col">
      <div class="flex capitalize justify-between mb-3" id="title">
        <p class="font-bold mb-0">Add a new Project</p>
        <button @click="addProject = !addProject" class="focus:outline-none">
      ...
      <div class="grid mt-5 pt-5 grid-cols-2 gap-4">
        <v-text-field
          ...
          clearable
          :label="newProject.startDate.label"
          :type="date"
          v-model="newProject.startDate.value"
          dense
        ></v-text-field>
        ...
        <v-text-field
          ...
          item-color="DPGREEN-500"
          clearable
          :label="newProject.deadline.label"
          :type="date"
          v-model="newProject.deadline.value"
          dense
        ></v-text-field>
      </div>
      <div class="mt-5">
      ...
      <div class="grid mt-5 pt-5 grid-cols-2 gap-4">
        <v-select
          ...
          item-color="DPGREEN-500"
          clearable
          :label="newProject.status.label"
          v-model="newProject.status.value"
          dense
          :items="Status"
          item-text="name"
          item-value="id"
        ...
        <v-select
          ...
          item-color="DPGREEN-500"
          clearable
          :label="newProject.priority.label"
          v-model="newProject.priority.value"
          :items="Priority"
          item-text="name"
          item-value="id"
          dense
...
import useAddProject from "~/controller/Project/addProject";
...
export default defineComponent({
  ...
  setup(props) {
    const { newProject, allColor, Status, Priority } = addNewProject();

    console.log("newProject>>>>>>>>>>>>>>>>>", newProject);

    const { addProject } = TableProject();
    const { addNewProjects } = useAddProject();

    const submit = () => {
      // validation(form).then((res: any) => {
      //   if (res) {
      console.log("sunbmit ptohrct is called");
      addNewProjects();
      // fetchClient();
    ...
    return { newProject, filterDropdown, allColor, addProject, remove, submit, Status, Priority };
  },
});
</script>
